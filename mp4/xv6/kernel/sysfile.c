  ...
  if(omode & O_CREATE){
    ...
    for (int i = 0; i <= MAXDEPTH; i++) {
      ...
      if (readi(ip, 0, (uint64)path, 0, MAXPATH) < 0) {
        // Cannot get the target name of the symbolic link
        iunlockput(ip);
        end_op();
        return -1;
      }
      ...
      iunlockput(ip);
      if ((ip = namei(path)) == 0){
        // Cannot get the inode of the target
        end_op();
        return -1;
      }
      ilock(ip);
...
sys_chdir(void)
{
  ...
  if (ip->type == T_SYMLINK) {
    // follow symlink recursively for no more than 10 times
    for (int times = 0; times < 11; ++times) {
      ...
      // break if target is not symlink
      if (ip->type != T_SYMLINK)
        break;
    }
  }
...
sys_symlink(void)
{
  ...
  if ((tlen = argstr(0, target, MAXPATH)) < 0 ||
      (plen = argstr(1, path, MAXPATH)) < 0 ||
      (ip = create(path, T_SYMLINK, 0, 0)) <= 0) {
    end_op();
    return -1;
  }
/*
  char nb = '\0'; // add '\0' at the end
  if (writei(ip, 0, (uint64)target, 0, tlen) != tlen ||
      writei(ip, 0, (uint64)&nb, tlen, 1) != 1) { */
